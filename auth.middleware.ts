    ...
    return new Elysia({ name: "auth-middleware" })
        .derive({ as: "global" }, async ({ headers }) => {
            ...
            if (authHeader && authHeader.startsWith("Bearer ")) {
                ...
                try {
                    const rawKey = env.JWT_PUBLIC_KEY_BASE64?.trimStart() || "";
                    const decoded = rawKey.startsWith("-----BEGIN")
                        ? rawKey
        ...
        .onBeforeHandle({ as: "global" }, async ({ headers, set, user }) => {
            ...
            try {
                ...
                if (allowedRoles && allowedRoles.length > 0) {
                    ...
                    const roleEntries = payload.role as
                        | Array<{ role: { role: string } }>
                        | undefined;
                    const userRoles =
                        roleEntries
                    ...
                    if (!hasPermission) {
                        console.log(
                            "authMiddleware: Insufficient permissions. Required:",
                            allowedRoles,
                            "Got:",
                            userRoles,
                        );
